...
const uploadBlobViaBackend = async (
  ...
  setChatMediaStatus('Enviando arquivo com validação segura...');
  ...
  if (!downloadURL) {
    throw createUploadError(response.status, payload, 'Upload concluído sem URL retornada.');
  }
...
const acceptRequest = async (requestId) => {
  ...
  try {
    ...
    addChatMessage({ author: 'Sistema', text: `SessÃ£o ${requestId} aceita com sucesso.`, kind: 'system' });
...
const uploadCustomProfilePhoto = async (file) => {
  ...
  if (!downloadURL) {
    throw createUploadError(response.status, payload, 'Upload da foto concluÃ­do sem URL.');
  }
  return downloadURL;
};
